  ...
  const calculateProbability = (article: ArticleProduction) => {
    ...
    const totalCapacityInTimeRemaining = machines.reduce((total, machine) => {
      ...
      if (!(machineName in machineCapacities)) {
        console.warn(
          `La machine "${machineName}" n'a pas de capacité définie dans machineCapacities.`,
        );
      ...
      {prod?.map((article) => {
        ...
        const percentage =
          article.objective > 0
            ? Math.min(
                Math.round(
                  (article.total_quantity_valid / article.objective) * 100,
                ),
                100,
              )
        ...
        const articlesRemaining = Math.max(
          0,
          article.objective - article.total_quantity_valid,
        );
        return (
          <div className="card" key={article.id}>
            <div className="card-article">
              ...
              <span className="colored-span">{article.objective} pièces</span>
            </div>
            <Segment color="red" className="progressbar-segment">
              <Progress
                ...
                size="big"
                className={
                  starts ? "progressbar-progress-start" : "progressbar-progress"
                }
                active={starts}
              />
            </Segment>
            <p className="probability">
              À ce rythme, nous estimons à <span>{probability}</span> %, la
              chance d'atteindre l'objectif à l'heure
            </p>
          </div>
        );
      })}
