...
function ProdDetail({
  ...
  const [productionToDelete, setProductionToDelete] = useState<number | null>(
    null,
  ); // ID de la production à supprimer
  ...
  const [editingMachineName, setEditingMachineName] = useState<string | null>(
    null,
  ); // Nom de la nouvelle machine éditée
  const [editingMachineNewId, setEditingMachineNewId] = useState<number | null>(
    null,
  ); // ID de la nouvelle machine éditée
  ...
  const [selectedNewMachine, setSelectedNewMachine] = useState<string | null>(
    null,
  );
  ...
  const [editingProduction, setEditingProduction] = useState<number | null>(
    null,
  );
  const [productionss, setProductionss] = useState<
  ...
  const handleMachineSelect = (
    ...
    setSelectedNewMachineId(
      selectedOption ? (selectedOption.key as number) : null,
  ...
  const handleMachineSelectModif = (
    ...
    setEditingMachineNewId(
      selectedOption ? (selectedOption.key as number) : null,
    ); // ID de la nouvelle machine modifiée
  };
  ...
  const handleNewEntry = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError(
          "Une erreur est survenue lors de l'enregistrement de la production.",
        );
      }
    }
  };
  ...
  const createProduction = async (
    ...
    if (!quantity || !machineId || !articleId) {
      setError(
        "Veuillez vérifier les informations de production (quantité, machine, article).",
      );
      return;
    }
  ...
  const handleQuantityChange = (productionId: number, value: string) => {
  ...
  const updateQuantity = async (
    ...
    const productionToUpdate = productionss.find(
        ...
        prevProductions.map((prod) =>
          prod.id === productionId
            ? {
                ...
                machines: {
                  id: newMachineId,
                  name: editingMachineName || "Non spécifiée",
                },
              }
            : prod,
        ),
  ...
  const articleImage = articleData
    ? getImageForArticle(articleData.nameArticle.name.replace(/\s+/g, "-"))
    : null;
  return (
          ...
          <div className="produit-details">
            <div className="background-image-prod">
              ...
              <div className="text-overlay">
                ...
                <p>
                  {machinesProduct.map((machine) => machine.name).join(" ")}
                </p>
              </div>
            </div>
                    ...
                    <label className="item-label">
                      ...
                      <form
                        ...
                        onSubmit={(e) => {
                          ...
                          const currentQuantity =
                            production.quantity_product_aff; // Quantité actuelle
                          const quantityToUpdate = Number.parseInt(
                            editingQuantities[production.id] ??
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            handleQuantityChange(production.id, e.target.value)
                          } // Appeler la fonction de changement de quantité
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditingQuantities((prev) => ({
                                ...prev,
                                [production.id]:
                                  production.quantity_product_aff.toString(), // Remplir avec la valeur actuelle
                              }));
                            }}
                          >
                            <Edit size={20} />
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEditingMachineName(
                                production.machines?.name || null,
                              );
                              setEditingMachineNewId(
                                production.machines?.id || null,
                              );
                            }}
                          >
                            <Edit size={20} />
