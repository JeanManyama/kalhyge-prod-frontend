...
const MachineDetail = ({  openMachineDetail, setOpenMachineDetail }: MachineDetailProps) => {
  ...
  const images: Record<string, { default: string }> = import.meta.glob("../../assets/*.png", { eager: true });
  ...
  const fetchMachineData = useCallback(async (timerId:number) => {
  ...
  const machineImage = machineData ? getImageForMachine(machineData.name.replace(/\s+/g, '-')) : null;
        ...
        <ModalDescription>
          <div className="machinedetailCard">
            ...
            <div className="background-image-machine">
              {machineImage ? (
                <img src={machineImage} alt={machineData?.name} className="machine-image" />
              ) : (
            ...
            <div className="machinedetailCard-form">
              {machineData?.articles.map((article) => {
                ...
                const totalProduction = article.total_quantity.length > 0 
                ? Math.max(...article.total_quantity) 
                : 0; 
                  // Le plus grand nombre du rejet
                  const totalRejection = article.total_quantity_reject.length > 0 
