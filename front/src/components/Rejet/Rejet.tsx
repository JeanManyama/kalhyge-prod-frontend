...
function Rejet({ setOpenRejet, openRejet }: RejetProps) {
    ...
    if (openRejet) {
      axios
        ...
        .then((response) => {
          ...
          const machineOptions = response.data.allMachines.map(
      ...
      setRejects((prevRejects) =>
        prevRejects.map((reject) =>
          reject.id === id
            ? {
                ...reject,
                ...(payload.actionType === "updateQuantity" && {
                  quantity_reject_aff: payload.newQuantity,
                }),
                ...(payload.actionType === "updateArticle" && {
                  articles: {
                    id: payload.articleId,
                    name:
                      articles.find((a) => a.key === payload.articleId)?.text ||
                      "Article inconnu",
                  },
                }),
                ...(payload.actionType === "updateMachine" && {
                  machines: {
                    ...
                    name:
                      machines.find((m) => m.key === payload.machineId)?.text ||
                      "Machine inconnue",
                  },
                }),
              }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setRejects((prevRejects) =>
        prevRejects.filter((reject) => reject.id !== deletingReject),
      );
  ...
  const handleSaveReject = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const selectedArticleDetails = articles.find(
        (option) => option.key === articleId,
      );

      // Trouver les détails de la machine sélectionnée
      const selectedMachineDetails = machines.find(
  ...
  return (
    ...
    >
              ...
              <ul>
                {rejects.map((reject) => (
                  <li key={reject.id} className="item">
                    <span>
                      {new Date(reject.created_at).toLocaleTimeString()}
                    </span>
                    ...
                    {editingProduction === reject.id ? (
                      ...
                      <form
                        onSubmit={(e) => {
                          ...
                          handleSaveEdit(reject.id, {
                            actionType: "updateQuantity",
                            newQuantity: Number.parseInt(
                              editingQuantities[reject.id],
                            ),
                          }).then(() => {
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            handleQuantityChange(reject.id, e.target.value)
                          }
                        />
                    ...
                    {editingArticle[reject.id] !== undefined ? (
                      <FormSelect
                        ...
                        onChange={(_e, { value }) =>
                          handleArticleChange(reject.id, value as number)
                        }
                      />
                    ) : (
                    ...
                    {editingMachine[reject.id] !== undefined ? (
                      <FormSelect
                        ...
                        onChange={(_e, { value }) =>
                          handleMachineChange(reject.id, value as number)
                        }
                      />
                    ) : (
      ...
      {deletingReject !== null && (
        <Modal open={true} size="tiny" onClose={cancelDelete}>
          <ModalHeader>Confirmer la suppression</ModalHeader>
          <ModalContent>
            <p>
              Êtes-vous sûr de vouloir supprimer ce rejet ? Cette action est
              irréversible.
            </p>
          </ModalContent>
          <ModalActions>
