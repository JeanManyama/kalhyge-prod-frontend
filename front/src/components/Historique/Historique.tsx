...
function Historique({ setOpenHistorique, openHistorique }: HistoriqueProps) {
  const [searchDate, setSearchDate] = useState<string>("");
  const [filteredArticles, setFilteredArticles] = useState<HistoriqueArticle[]>(
    [],
  );
  const [filteredMachines, setFilteredMachines] = useState<HistoriqueMachine[]>(
    [],
  );
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const response = await axios.post(`${apiUrl}/productions/historique`, {
        date: searchDate,
        period: period,
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      setError("");
      const response = await axios.delete(`${apiUrl}/productions/historique`, {
